...
export default function AdminMatchesClient({ initialMatches }: { initialMatches: any[] }) {
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-lg text-xs font-black uppercase tracking-wider shadow-xl border ${toast.type === 'success' ? 'bg-gaming-green/10 border-gaming-green/30 text-gaming-green' : 'bg-red-950/50 border-red-800/50 text-red-400'
          }`}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="flex gap-2">
        {(['all', 'upcoming', 'live', 'finished'] as const).map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border transition-all ${filter === f
